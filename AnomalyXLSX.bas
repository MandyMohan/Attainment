  ...
  With xArray(1)
    ...
    With ActiveSheet
        ...
        With Range("B10:B11")
            ...
            .VerticalAlignment = xlCenter
        End With
        .Range("A8:B8,A10:C11").Interior.Color = RGB(221, 235, 247)
        With Range("B12:B21")
            .NumberFormat = "0"
            .Value = .Value
        End With
    ...
    With ActiveChart
        ...
        If Sheets("Graph").Range("F1") = "Victoria" Or Sheets("Graph").Range("F1") = "Caroni" Or Sheets("Graph").Range("F1") = "Tobago" Or Sheets("Graph").Range("F1") = "St Patrick" Then
            .Parent.Width = Sheets("Graph").Range("A25:F25").Width - 125
            .Shapes("Rectangle 1").Width = Sheets("Graph").Range("A25:F25").Width - 135
        Else
            .Parent.Width = Sheets("Graph").Range("A25:G25").Width + 50
            .Shapes("Rectangle 1").Width = Sheets("Graph").Range("A25:G25").Width + 30
        End If
...
'add chart to second sheet in array
 ...
 For i = 3 To 12
    ...
    last = ActiveSheet.Cells(Rows.Count, "A").End(xlUp).Row
    ActiveSheet.Range(Cells(last, 1), Cells(last, 3)).Value = ActiveSheet.Range(Cells(last - 1, 1), Cells(last - 1, 3)).Value
    ActiveSheet.Range("D" & last).Value = "TOTAL"
    ActiveSheet.Range(Cells(last, 5), Cells(last, 15)).Copy
    ActiveSheet.Range(Cells(last, 5), Cells(last, 15)).PasteSpecial Paste:=xlPasteValues
    With ActiveSheet
